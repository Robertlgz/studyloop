<template>
    ...
    >
        ...
        <div class="sl-popup-footer" v-if="results.length > 1">
            <span class="sl-footer-count">{{ results.filter(r=>r.ok).length }}/{{ results.length }} 个源有结果</span>
...
async function onSearch(evt: CustomEvent) {
    ...
    try {
        ...
        if (results.value.length === 0) {
            error.value = `未找到 "${word.value}" 的释义。已启用：${enabled.join(", ")}`;
        }
    } catch (e) {
...
async function addToDeckFromSource(engineId: string) {
    ...
    const newWord: Word = {
        ...
        pos: "", status: 1, t: "WORD", language: "en",
        tags: [engineId], notes: [], sentences: [],
        date: Math.floor(Date.now() / 1000), mastery: 1,
        mdLink: null, exposures: 0, lastExposure: null,
        exposureHistory: [], fsrs: createInitialCard(),
        ankiNoteId: null, ankiHash: undefined,
    };
...
.sl-btn-add {
    background: white; color: var(--interactive-accent);
...
.sl-btn-close {
    ...
    color: white; font-size: 1em; opacity: 0.8; padding: 4px;
}
...
.sl-spinner {
    ...
    border-radius: 50%; animation: sl-spin 0.6s linear infinite;
...
.sl-btn-sm {
    padding: 3px 10px; font-size: 0.77em;
    background: var(--background-secondary); border: 1px solid var(--background-modifier-border);
    border-radius: 5px; cursor: pointer; color: var(--text-normal); transition: background 0.1s;
}
